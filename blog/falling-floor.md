...
Every other constraint in this business is a ceiling. Compute has one. Energy has one. Data has one. This is the only input that is a **floor collapsing**, and it is the most consequential number in the entire forecast.
...
|---|---|---|
| $/token falling | AI is unprofitable | Volume explodes; spend rises anyway |
...
The third row is where most of the money is being lost, so it is worth being blunt about the mechanism. Margin compression at the application layer is not evidence that something broke. It is the system working as designed, arriving on time.
...
|---|---|---|
| **Hardware** | $/FLOP plus serving-stack utilization | Manufacturing cadence. Steady, slow, unlikely to stop, unlikely to accelerate |
...
| **Competition** | Several near-equivalent vendors converting cost into price | The fragile one. This is a market-structure fact, not a technical one |
...
Costs will almost certainly keep falling — hardware and distillation are grinding away regardless of who wins. Whether *prices* follow is a question about market structure, and market structure is exactly the thing under active contest. Consolidation to three to five frontier labs, plus regulatory moats that raise the cost of being a new entrant, would restore pricing power without a single technical change.

So the two-year moat is a **cost claim** that only becomes a **product-strategy claim** when competition transmits it into price. Score serving costs and serving prices as two separate series. The scenario where the moat survives technically but dies commercially is the [labs endgame](../02-games/1-labs.md), and it is not remote.

The current evidence runs the other way, though. Open-weight models sit only about **3–6 months behind the frontier** — Epoch AI's January–May 2026 comparison puts the average lag near **four months**, roughly six under a stricter comparison rule. That figure has been *shortening*. Earlier versions of this analysis carried 9–15 months. Fast-following is keeping the model layer competitive, which is why surplus is passing *through* the model layer rather than pooling in it.

## The collapse is uneven, and the unevenness is predictable

The headline number hides a gradient. Prices fall fastest exactly where models are substitutable — which, by the [verification asymmetry](../01-substrate/data.md), means the benchmarkable work. Where output quality is hard to verify, buyers cannot compare vendors, switching carries real risk, and price stays sticky.
...
## The question nobody's planning question actually is

If capability is free on a two-year lag, then the right planning question is never *what can the best model do today*.

It is: **what will be free in 2028, and what will still be scarce then?**

Almost all product strategy in this industry answers the first question. The scarce list is short and boring and has barely moved: energy, interconnection, licenses, trusted real-time data, distribution, liability capacity, physical presence, verified identity, and senior judgment after the apprenticeship gap does its damage.

Notice that six of those are owned by capital rather than earned by labor. That is a separate and uncomfortable argument, made [elsewhere in the corpus](../09-macro/assets.md).

## What would falsify this

State it plainly, because a claim that cannot be caught failing is marketing:

- **Inference costs stop falling** at fixed capability — an architectural wall, an energy floor, or deliberate restriction.
