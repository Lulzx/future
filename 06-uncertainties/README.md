...
← [Index](../README.md) · [Part V](../05-probabilities/README.md)

---

Three live uncertainties, in **descending order of variance**. Everything else in this document sits on firmer ground; these are where it would actually be revised.
...
Note that all three are skewed in the same direction: **each is more likely to make the world move faster or turn out better than the base case, rather than slower or worse.** That is worth stating explicitly, because a document that spends most of its length on constraints and harms can read as pessimistic when the error bars are actually asymmetric the other way.
...
I have deliberately left both sides of this in rather than resolving it. It is genuinely unresolved.
